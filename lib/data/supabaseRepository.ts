...
export class SupabaseDataRepository implements IDataRepository {
  ...
  async addMessage(message: Omit<ChatMessage, 'id' | 'createdAt'>): Promise<ChatMessage> {
    ...
    if (!message.tourId || !message.userId) {
      throw new Error('Tour ID and User ID are required')
    }
  ...
  async markTourAsRead(tourId: string, userId: string, timestamp: Date | string): Promise<boolean> {
    try {
      // Validate inputs
      if (!tourId || !userId || typeof tourId !== 'string' || typeof userId !== 'string') {
        return false
      }
  ...
  async getLastReadTimestamp(tourId: string, userId: string): Promise<Date | null> {
    try {
      ...
      if (error) {
        ...
        // PGRST301 = JWT expired (handled by handleSupabaseError)
        // 42P01 = relation does not exist (table not created yet)
        // 42501 = insufficient_privilege (RLS policy blocking)
        // 22P02 = invalid input syntax for type uuid (invalid UUID format)
        if (
          error.code === 'PGRST116' || 
          error.code === '42P01' || 
          error.code === '42501' ||
          ...
          // No record found, table doesn't exist, permission denied, or invalid UUID - not read yet
          return null
        }
        if (!this.handleSupabaseError(error)) {
          ...
          console.error('Error getting last read timestamp:', errorDetails)
          
          // If it's an RLS policy error or permission error, just return null (not read yet)
          if (error.code === '42501' || error.message?.includes('permission') || error.message?.includes('policy')) {
            console.warn('RLS policy may be blocking access, returning null')
            return null
          }
        }
        return null
      ...
      // If no data, return null (not read yet)
      if (!data || !data.last_read_at) return null
      ...
      const errorInfo = {
        ...
        stack: err?.stack,
        name: err?.name,
        fullError: err ? JSON.stringify(err, Object.getOwnPropertyNames(err)) : 'Unknown error',
      }
      console.error('Unexpected error in getLastReadTimestamp:', errorInfo)
      return null
    }
  }
  ...
  async getUnreadCount(tourId: string, userId: string): Promise<number> {
    try {
      ...
      console.error('Unexpected error in getUnreadCount:', errorInfo)
      return 0
    }
  }
  ...
  async uploadGpxFile(tourId: string, file: File): Promise<string> {
    ...
    // This ensures the file has the correct type property
